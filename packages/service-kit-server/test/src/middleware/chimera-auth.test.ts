...
import authentication, { AXIOS_TIMEOUT_CODE, client } from '../../../src/middleware/chimera-auth';
...
describe('Given the authentication middleware', () => {
  ...
  describe('when auth is enabled but not required', () => {
    describe('when credentials are specified', () => {
      ...
      beforeAll(async () => {
        getStub.mockResolvedValue({ status: 200 });
        xmlStub.mockResolvedValue({ token: { memberId: 'mock-member-id' } });
        await authentication(mockConfig)(mockContext, mockNext);
      });
  ...
  describe('when auth is enabled and required', () => {
    ...
    describe('when the memberIds do not match', () => {
      const mockContext: Context = {
        ...baseContext,
        contract: mockAuthRequiredSpec
      } as Context;
      ...
      beforeAll(async () => {
        getStub.mockResolvedValue({ status: 200 });
        xmlStub.mockResolvedValue({ token: { memberId: '123' } });
        try {
          await authentication(mockConfig)(mockContext, mockNext);
        } catch (err) {
      ...
      it('should call respond with the correct error payload', () => {
        expect(errorThrown).not.toBeInstanceOf(ServiceKitApplicationError);
        expect(errorThrown.message).toEqual('Invalid details');
      });
